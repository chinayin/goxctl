...
- **调试输出**：`GOXCTL_DEBUG=1`、`goxctl --verbose <cmd>` 或 `-v` 打印解析的 owner/repo、release URL、命中资产、转发 exec 等。环境变量贯穿转发链（扩展子进程继承）；核心 `--verbose`/`-v` 写回该变量，且**扩展自身也全局支持 `--verbose`/`-v`**，故 `goxctl --verbose claude update`（前置）与 `goxctl claude update --verbose`（后置）都生效。短旗三件套：`-v`=verbose、`-V`=version、`-h`=help（version 预注册同名 bool flag 以抢占 `-V` 短旗）。
- **输出风格**：列表用对齐表格（大写表头），末列次要信息（如 module 路径）暗色；操作前打印进度行 `Installing/Pulling ...`、成功用 `✓`（TTY 上色，管道自动无色），错误用 `error:`；统一无静默成功（参照 gh/docker/brew）。`--help` 列表去掉 `completion`/`help` 噪音（功能仍在）。
